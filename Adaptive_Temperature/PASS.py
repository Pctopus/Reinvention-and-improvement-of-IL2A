...
class protoAugSSL:
    ...
    def __init__(self, args, file_name, feature_extractor, task_size, device):
        ...
        self.initial_temp = args.temp  # 初始温度参数
        self.initial_kd_weight = args.kd_weight  # 初始知识蒸馏损失权重
        self.initial_protoAug_weight = args.protoAug_weight  # 初始原型增强损失权重
    ...
    def train(self, current_task, old_class=0):
        ...
        for epoch in range(self.epochs):
            for step, (indexs, images, target) in enumerate(self.train_loader):
                ...
                opt.zero_grad()  # 清零梯度
                loss = self._compute_loss(images, target, old_class, epoch)  # 计算损失
                opt.zero_grad()
                loss.backward()  # 反向传播
                opt.step()  # 优化更新
    ...
    def _compute_loss(self, imgs, target, old_class=0, epoch=None):
        # 动态调整温度参数和权重
        current_temp = max(0.1, self.initial_temp * (1 - epoch / self.epochs))
        current_kd_weight = self.initial_kd_weight * (epoch / self.epochs)
        current_protoAug_weight = self.initial_protoAug_weight * (1 - epoch / self.epochs)
        ...
        if self.old_model is None:
            ...
            loss_protoAug = nn.CrossEntropyLoss()(soft_feat_aug / current_temp, proto_aug_label)  # 使用当前的温度参数

            # 最终的损失是分类损失、原型增强损失和知识蒸馏损失的加权和
            return loss_cls + current_protoAug_weight * loss_protoAug + current_kd_weight * loss_kd
