...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.non_local = NonLocalBlock(planes)  # 添加非局部注意力机制
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.sa = SpatialAttention()  # 空间注意力机制
        self.non_local = NonLocalBlock(planes * 4)  # 添加非局部注意力机制
...
class NonLocalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        g_x = g_x.permute(0, 2, 1)  # (batch_size, H*W, inter_channels)

        theta_x = self.theta(x).view(batch_size, self.inter_channels, -1)
        theta_x = theta_x.permute(0, 2, 1)  # (batch_size, H*W, inter_channels)

        phi_x = self.phi(x).view(batch_size, self.inter_channels, -1)  # (batch_size, inter_channels, H*W)

        f = torch.matmul(theta_x, phi_x)  # (batch_size, H*W, H*W)
        f_div_C = F.softmax(f, dim=-1)

        y = torch.matmul(f_div_C, g_x)  # (batch_size, H*W, inter_channels)
        y = y.permute(0, 2, 1).contiguous()  # (batch_size, inter_channels, H*W)
        y = y.view(batch_size, self.inter_channels, H, W)  # (batch_size, inter_channels, H, W)
        W_y = self.W(y)  # (batch_size, in_channels, H, W)
        z = W_y + x
